...
class JavaClassFinder(
    ...
    fun findClassFiles(classOrJarOrDirectory: File): Set<JavaClass> = findClassFiles(listOf(classOrJarOrDirectory))

    fun findClassFiles(classOrJarOrDirectories: List<File>): Set<JavaClass> = classOrJarOrDirectories
        .flatMap { classOrJarOrDirectory ->
            findClassParserDeep(classOrJarOrDirectory)
                .mapNotNull { parser ->
                    try {
                        parser.parse()
                    } catch (e: IOException) {
